...
class HostFingerprintAdapter(ABC):
    ...
    @abstractmethod
    def ensure_platform(self, config: RotundaProfile) -> None:
        raise NotImplementedError

    @abstractmethod
    def ensure_oscpu(self, config: RotundaProfile) -> None:
        raise NotImplementedError

    def adjust_generated_screen(self, screen: ScreenFingerprint) -> None:
        _ = screen

    def normalize_screen(self, config: RotundaProfile) -> None:
        _ = config
    ...
    def finalize_config(self, config: RotundaProfile) -> None:
...
def current_host_target_os() -> HostTargetOS:
    try:
        ...
        raise NotImplementedError(
            "Rotunda fingerprinting currently ships host adapters only for macOS and Linux."
        ) from error
...
def normalize_target_os(value: Any | None) -> HostTargetOS:
    ...
    for candidate in candidates:
        if candidate != host_target_os:
            raise NotImplementedError(
                "Rotunda fingerprinting currently supports only the real host OS. "
...
def _merge_seed_values(config: RotundaProfile) -> None:
...
def _merge_host_inventories(config: RotundaProfile, host: HostFingerprintAdapter) -> None:
    config.fonts = config.fonts or FontsProfile()
