...
from ._generated_profile import LocaleProfile, NavigatorProfile, RotundaProfile
...
def generate_context_fingerprint(
    ...
    the current host OS and Rotunda normalizes it into a smaller
    ...
    - `config`: the final generated RotundaProfile
...
def generate_fingerprint(
    ...
    **config: Any,
    ...
    `from_browserforge()` to compile it into a Rotunda config map.
    """
...
def from_browserforge(fingerprint: Fingerprint, ff_version: str | None = None) -> RotundaProfile:
    """
    Compile a BrowserForge fingerprint into a host-compatible `RotundaProfile`.
...
def from_preset(preset: dict[str, Any], ff_version: str | None = None) -> RotundaProfile:
    """
    Compile an explicit caller-supplied preset into a host-compatible `RotundaProfile`.

    This path exists for callers that already have a preset dictionary and want
    Rotunda to normalize it the same way as BrowserForge output. Rotunda no
...
def is_generated_fingerprint(fingerprint: Fingerprint) -> bool:
    """
    Return whether a fingerprint was produced by Rotunda's BrowserForge helper.
    """
...
def _debug_log(enabled: bool, message: str) -> None:
    if enabled:
        print(f"[rotunda:fingerprint] {message}")


def _apply_locale_override(config: RotundaProfile, locale: str) -> None:
