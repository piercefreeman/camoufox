...
def get_asset_by_name(name: str) -> Path:
    """
    Return the filesystem path for a packaged Camoufox asset.
    ...
    try:
        ...
        raise FileNotFoundError(f'Unknown Camoufox asset "{name}". Available: {available}') from error

    if not asset_path.is_file():
        raise FileNotFoundError(f"Camoufox asset is missing from the package: {asset_path}")
    return asset_path
