...
import pytest
import torch
from rotunda_models.constants import KEY_BACKSPACE, KEY_STOP, KEY_UNKNOWN_ACTION
...
def test_keyboard_target_support_allows_unknown_action_stand_in() -> None:
    action_to_id = {"a": 0, KEY_UNKNOWN_ACTION: 1, KEY_BACKSPACE: 2, KEY_STOP: 3}

    require_keyboard_target_supported("", "aΩ", action_to_id)


def test_structured_decode_materializes_unknown_action_to_target_character() -> None:
    char_to_id = {"<PAD>": 0, "<UNK>": 1, "<EOS>": 2, "<SEP>": 3}
    ...
    assert [row["action"] for row in rows] == ["Ω"]
    assert rows[-1]["textAfter"] == "Ω"
...
def test_learned_typo_head_can_choose_reachable_wrong_character() -> None:
    ...
    with torch.no_grad():
        ...
        model.press_count_head.bias.fill_(math.log(3.0))
    checkpoint = {
        "kind": "keyboard_action_gru",
        "model_config": model_config,
        "char_to_id": char_to_id,
...
def test_constrained_decode_uses_predicted_press_budget() -> None:
    char_to_id = {"<PAD>": 0, "<UNK>": 1, "<EOS>": 2, "<SEP>": 3, "a": 4, "x": 5}
    action_to_id = {"a": 0, "x": 1, KEY_BACKSPACE: 2, KEY_STOP: 3}
    id_to_action = {index: action for action, index in action_to_id.items()}
    model_config = {
...
def test_constrained_decode_requires_press_count_head() -> None:
    char_to_id = {"<PAD>": 0, "<UNK>": 1, "<EOS>": 2, "<SEP>": 3, "a": 4}
    action_to_id = {"a": 0, KEY_BACKSPACE: 1, KEY_STOP: 2}
    id_to_action = {index: action for action, index in action_to_id.items()}
    model_config = {
        "char_vocab_size": len(char_to_id),
        "action_vocab_size": len(action_to_id),
    ...
    with pytest.raises(RuntimeError, match="predictive press-count head"):
        decode_keyboard_rows(
            checkpoint=checkpoint,
            model=model,
            final_string="a",
